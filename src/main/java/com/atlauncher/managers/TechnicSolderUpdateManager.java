...
public class TechnicSolderUpdateManager {
    ...
    public static void checkForUpdates() {
        ...
        LogManager.info("Checking for updates to Technic Solder instances");

        boolean refreshInstancesPanel = Data.INSTANCES.parallelStream().filter(i -> i.isTechnicSolderPack()).map(i -> {
            ...
            if (Data.TECHNIC_SOLDER_INSTANCE_LATEST_VERSION.containsKey(i)
                    && !Data.TECHNIC_SOLDER_INSTANCE_LATEST_VERSION.get(i).latest.equals(technicSolderModpack.latest)) {
                wasUpdated = true;
            }
