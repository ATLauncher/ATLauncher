...
public class CheckingServersManager {
    private static final Logger LOG = LogManager.getLogger(CheckingServersManager.class);
    ...
    public static void loadCheckingServers() {
        ...
        LOG.debug("Loading servers to check");
        ...
        if (Files.exists(FileSystem.CHECKING_SERVERS_JSON)) {
            ...
            if (fileReader != null) {
                try {
                    ...
                    LOG.error("Exception while trying to close FileReader when loading servers for server "
                                    + "checker" + " tool.", e);
                }
            }
        }

        LOG.debug("Finished loading servers to check");
