...
public class ShaderPacksSection extends SectionPanel {
    private JTextField searchField = new JTextField(16);
    private TableModel tableModel;

    public ShaderPacksSection(Instance instance) {
        super(instance);
    ...
    private void setupComponents() {
        ...
        List<Path> modPaths = instance.getModPathsFromFilesystem(
                Arrays.asList(instance.ROOT.resolve("shaderpacks")));

        Object[][] modsData = modPaths.stream().map(path -> {
            DisableableMod mod = instance.launcher.mods.parallelStream()
                    .filter(m -> m.type == Type.shaderpack && m.file.equals(path.getFileName().toString())).findFirst()
                    .orElseGet(() -> {
                        LogManager
                                .warn(String.format(
                                        "Failed to find shaderpack for file %s. Generating temporary mod for it.",
                                        path.getFileName().toString()));
                        return DisableableMod.generateMod(path.toFile(), Type.shaderpack, !path.endsWith(".disabled"));
                    });
        ...
        sideBar.add(new JButton(GetText.tr("Download Shader Packs")));
